...
const controller = {
	...
	animateProjectScroll(content) {
		...
		*/
		...
		TweenMax.set(content, {height: "84vh", onComplete: () => {
	...
	initSlide(module) {
		...
		TweenMax.to(description, 0.6, {opacity: 1, visibility: 'visible', delay: 0.6 });
	...
	toggleActiveStyles(parent) {
		...
		*/
		...
		TweenMax.set(".project-item .content", {height: "60vh", opacity: 1, scale: 1});	
		TweenMax.set(".module-injection", {opacity: 0, visibility: 'hidden', height: 0});
		TweenMax.to('.module-injection .project-content', 0.6, {opacity: 0, visibility: 'hidden'});
	},
	smoothCloseSlide() {
		...
		*/
		...
		$('.next.item i').attr('class', 'fa fa-arrow-right');	
		TweenMax.to(".project-item .content", 0.2, {height: "60vh", opacity: 1, scale: 1});	
